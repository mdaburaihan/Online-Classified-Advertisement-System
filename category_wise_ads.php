<?php 
$title="Category wise Ads | Online Classified Advertisement System";
...
<div class="container">
	<?php
	$getCategoryName=$cat->getCategoryById($catid);
	if($getCategoryName)
	{
		$catresult=$getCategoryName->fetch_assoc();
		?>

		<ul class="breadcrumb">
			<li><a href="index.php"><i class="fa fa-home"></i></a></li>
			<li><a href="#">Advertisements</a></li>
			<li><a href="#"><?php echo $catresult['cat_name'];?></a></li>
		</ul>
		...
		<div style="background-color: #9392ba;font-size: 18px;color:white;width:1000px;padding:10px 10px;">
			...
			<a class="btn btn-primary" data-toggle="collapse" href="#MoreCategoryCollapse" role="button" aria-expanded="false" aria-controls="MoreCategoryCollapse" style="background-color: #9392ba;font-size: 18px;padding:0px;">

			 <?php echo $catresult['cat_name']; ?> <i class="fa fa-caret-down"></i>
			 </a>
		</div>
        
		<?php
          $catid= $catresult['cat_id'];
	}
	?>
	<!-- Displaying more subcategory under the selected category -->
	 <div class="collapse in" id="MoreCategoryCollapse">
		<div style="padding-left: 20px;background-color: #f5f5f5;border: 1px solid #ebe5e5;font-size: 16px;color:#1b4f91"><i class="fa fa-bars"></i> Subcategories under Category <?php echo $catresult['cat_name']; ?></div>
			...
			if($getSubcatNamesByCat)
			{
				while($catselect=$getSubcatNamesByCat->fetch_assoc())
				{
					...
					<div class="col-lg-3" style="margin-top: 5px;">
 				    	<a href="subcategory_wise_ads.php?subcatid=<?php echo $catselect['subcat_id']; ?>" role="button" class="btn btn-sm form-control" style="background-color: #a0ba18;font-size: 16px"><?php echo $catselect['subcat_name']; ?></a>
					</div>
				<?php
				}
			}
	...
	<div class="so-basic-product">
		<div class="item-wrap row cf products-list grid" style="background-color: #e7ebff">
			...
			$getAdscatWise = $ads->getAdsCategoryWise($catid);
			if($getAdscatWise){
				while ($arr=$getAdscatWise->fetch_assoc()){
					?>
					<div class="col-lg-3">
